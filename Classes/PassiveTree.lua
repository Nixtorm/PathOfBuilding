...
local PassiveTreeClass = newClass("PassiveTree", function(self, treeVersion)
	...
	for nodeId, socket in pairs(sockets) do
		...
		for radiusID, radiusInfo in pairs(data[self.targetVersion].jewelRadius) do
			socket.nodesInRadius[radiusID] = { }
			socket.attributesInRadius[radiusID] = { }

			local rSq = radiusInfo.outerRadius * radiusInfo.outerRadius
			local irSq = radiusInfo.innerRadius * radiusInfo.innerRadius
			for _, node in pairs(self.nodes) do
				if node ~= socket and not node.isBlighted then
					...
					if vX * vX + vY * vY <= rSq and vX * vX + vY * vY >= irSq then
						socket.nodesInRadius[radiusID][node.id] = node
					end
				end
			end
		end
